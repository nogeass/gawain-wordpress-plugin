...
**What makes Gawain AI unique:**

* Brand-preserving AI — your product's identity stays intact in every frame
* Just 1 image + a simple prompt — the AI handles everything else
* Daily rotation — fresh videos with different creative angles every day
* Mass production + differentiation — scale your video content for SNS and ads without losing uniqueness

**Fully Automated Video Pipeline**

From a single product image, Gawain AI automatically performs:

* Concept extraction (e.g. "cute x moisturizing x summer")
...
Feature | Gawain AI Video | Conventional Tools (Lumen5, etc.)
Auto scene composition | Yes (industry-first) | No (manual editing required)
Daily creative rotation | Yes | No
Video from 1 image | Yes | Requires lengthy text input
Commercial / ad-ready | Yes | Requires copyright verification

**Use Cases**

* **D2C Brands** — Generate daily short-form commercials for cosmetics, food, fashion products
* **Campaign Videos** — Auto-generate product promotion shorts for each campaign
* **Store & Facility Promotion** — Produce introduction videos with zero manpower

**Core Plugin Features**

* One-click AI video generation from product images
* Video carousel widget on product pages (WooCommerce hook + `[gawain_videos]` shortcode)
...
「ガウェインAI」は、**ブランド商品の形を保ったまま**、世界に一つだけの"特別制作"プロモーション動画を全自動生成します。

**わずか1枚の画像と簡単なプロンプト入力だけで、AIが以下を全自動で行います：**

* コンセプトの抽出（例：「かわいい × 潤い × 夏」）
* シーン構成・カット分割（冒頭カット → 商品紹介 → ユーザー利用 → クロージング）
* テキスト生成・コピー演出
* 動画編集・アニメーション合成
* BGM・エフェクトの最適化

さらに、**毎日異なる切り口から動画を生成する日替わりプロモーション機能**を搭載。SNSやWeb広告での量産×差別化を同時に実現します。

**競合と比較して圧倒的な優位性**

機能 | ガウェインAI | 従来ツール（Lumen5等）
自動構成（シーン分割） | ○（業界初） | ×（手動編集が必要）
日替わり切り口生成 | ○ | ×
1枚画像からの動画生成 | ○ | △（長文テキストが必要）
商用利用・広告出稿対応 | ○ | △（著作権確認が必要）

**想定ユースケース**

* **D2Cブランド** — 化粧品や食品などのD2Cブランドによる毎日異なる短尺CM
* **キャンペーン動画** — キャンペーンごとに自動生成される商品訴求ショート動画
* **店舗・施設紹介** — 店舗・施設の紹介映像を人手ゼロで量産

---

= 한국어 =

**【업계 최초】브랜드 제품의 형태를 그대로 유지하는 AI 프로모션 영상 자동 생성**

기존 영상 제작 도구들은 템플릿 기반이거나 수작업 편집이 필요했습니다. 브랜드가 중시하는 제품의 외관과 세계관을 유지하기가 어려웠습니다.

가웨인 AI는 **브랜드 제품의 형태를 그대로 보존**하면서, 세상에 하나뿐인 맞춤형 프로모션 영상을 완전 자동으로 생성합니다.

**이미지 1장과 간단한 프롬프트만으로 AI가 자동 수행:**

* 컨셉 추출 (예: "귀여움 × 촉촉함 × 여름")
* 씬 구성 및 컷 분할 (오프닝 → 제품 소개 → 사용 장면 → 클로징)
* 카피라이팅 및 텍스트 연출
* 영상 편집 및 애니메이션 합성
* BGM 및 이펙트 최적화

**일일 로테이션 기능**으로 매일 다른 크리에이티브를 자동 생성하여 SNS 및 광고의 대량 제작과 차별화를 동시에 실현합니다.

**활용 사례**

* **D2C 브랜드** — 화장품, 식품 등 D2C 브랜드의 매일 다른 숏폼 광고
* **캠페인 영상** — 캠페인별 자동 생성 제품 프로모션 숏 영상
* **매장·시설 소개** — 인력 없이 소개 영상 대량 제작

---

= 中文 =

**【行业首创】保持品牌产品原貌的AI自动生成推广视频**

传统视频制作工具大多依赖模板套用或人工剪辑，难以保持品牌所重视的产品外观和品牌世界观。

Gawain AI 能够在**完整保留品牌产品形态**的同时，全自动生成独一无二的定制推广视频。

**仅需1张图片和简单的提示词，AI即可自动完成：**

* 概念提取（例："可爱 × 滋润 × 夏日"）
* 场景构成与分镜（开场 → 产品介绍 → 使用场景 → 结尾）
* 文案生成与文字特效
* 视频编辑与动画合成
* BGM与特效优化

搭载**每日轮换推广功能**，每天自动生成不同创意角度的视频，实现社交媒体和广告的批量制作与差异化并行。

**应用场景**

* **D2C品牌** — 化妆品、食品等D2C品牌的每日短视频广告
* **活动视频** — 按活动自动生成产品推广短视频
* **门店·设施介绍** — 零人力批量制作介绍视频
